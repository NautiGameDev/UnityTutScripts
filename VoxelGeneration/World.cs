...
public class World : MonoBehaviour
{
    ...
    private Task<ConcurrentDictionary<Vector3Int, ChunkData>> CalculateWorldChunkData(List<Vector3Int> chunkDataPositionToCreate)
    {
        ConcurrentDictionary<Vector3Int, ChunkData> dictionary = new ConcurrentDictionary<Vector3Int, ChunkData>();
        
        return Task.Run(() => 
        {
            foreach (Vector3Int pos in chunkDataPositionToCreate)
            {
    ...
    internal bool SetBlock(RaycastHit hit, BlockType blockType)
    {
        ...
        {
            List<ChunkData> neighborDataList = Chunk.GetEdgeNeighborChunk(chunk.ChunkData, pos);
            foreach (ChunkData neighborData in neighborDataList)
            {
                ChunkRenderer chunkToUpdate = WorldDataHelper.GetChunk(neighborData.worldReference, neighborData.worldPosition);
    ...
    {
        if (Mathf.Abs(pos % 1) == 0.5f) //Since each face is 0.5f from center
        {
            pos -= (normal / 2);
        }
    ...
    {
        foreach (var treeLeaf in chunkData.treeData.treeLeavesSolid)
        {
            Chunk.SetBlock(chunkData, treeLeaf, BlockType.TreeLeavesSolid);
        }
    }
